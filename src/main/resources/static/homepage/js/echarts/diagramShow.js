$(function(){
	var datas = "";
	//读取OIDData文件并进行修改
	$.getJSON("/homepage/js/echarts/oidData.json",function (data) {
		datas = data.data;
		for(var i in datas){
			var symbolSize = datas[i].symbolSize;
			if(symbolSize[0] == 60){
			    symbolSize[0] = 50;
			    symbolSize[1] = 50;
			}else if(symbolSize[0] == 40){
				symbolSize[0] = 35;
				symbolSize[1] = 35;
			}else{
				symbolSize[0] = 25;
	...
	var	option = {
	    ...
	    tooltip: {                  // 提示框的配置
	        formatter: function(param) {
	        	...
	        	if(param.data.registerTime != "" && param.data.registerTime != null && param.data.iphone != "" && param.data.iphone != null && param.data.address != "" && param.data.address != null){
	        		return "<p>名称:"+param.data.trueName+"</p><p>注册时间:"+param.data.registerTime+"</p><p>地址:"+param.data.address+"</p><p>电话号码:"+param.data.iphone+"</p>";
	        	}else if(param.data.trueName != "..."){
	        		return "<p>名称:"+param.data.trueName+"</p>"
	        	}else{
	        		return "";
	        	}
	        ...
	        label: {                // 关系对象上的标签
	            normal: {
	                ...
	                formatter:function(params){
	                	if(params.data.name == 3391){
	                		return 1
	                	}
	                	if(params.data.name == 1562){
	                		return "..."
	                	}
	                	if(params.data.name == 30012){
	                		return "..."
	                	}
	                	if(params.data.name == 32654){
	                		return "..."
	                	}
...
// 异步加载数据
$.get('/homepage/js/echarts/oidData.json').done(function (data) {
    // 填入数据
    chart.setOption({
        series: [{
            data: datas,
